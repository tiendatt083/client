    ...
    <div class="control-bar">
      ...
      <div class="ui input search-input">
        <input
          v-model="searchText"
          type="text"
          placeholder="Search English or Vietnam..."
          autocomplete="off"
        />
      </div>
    </div>
      ...
      <table class="ui celled compact striped selectable table wordlist-table">
        ...
        <tbody>
          ...
          <tr v-if="filteredWords.length === 0">
            <td colspan="6" style="text-align:center;color:#bbb;">No results found.</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const filteredWords = computed(() => {
  ...
  if (searchText.value) {
    ...
    arr = arr.filter(
      (word) =>
        ...
        (word.vietnam && word.vietnam.toLowerCase().includes(keyword))
    );
  }
  ...
  if (filterLetter.value) {
    arr = arr.filter(w =>
      (w.english && w.english[0]?.toUpperCase() === filterLetter.value) ||
      (w.vietnam && w.vietnam[0]?.toUpperCase() === filterLetter.value)
    );
  }
  ...
  return arr.slice().sort((a, b) => {
    const aStr = (a.english || '').toLowerCase();
    const bStr = (b.english || '').toLowerCase();
...
/* English & Vietnam text - align left, bold, icon inline */
.wordlist-table td:nth-child(2), .wordlist-table td:nth-child(3) {
  text-align: left;
  padding-left: 16px;
  font-weight: 500;
  color: #2a265e;
